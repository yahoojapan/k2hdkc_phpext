...
if(!file_exists("./tests/slave.ini")){
	...
	if(false !== $handle){
		...
		$subval = "subval";
		// 1. set
		k2hdkcpx_add_subkey($handle, $key, $subkey, $subval);
		time_nanosleep(0, 250 * 1000 * 1000);

		if ($subval === k2hdkcpx_get_value($handle, $subkey)) {
			// 2. clear
			k2hdkcpx_clear_subkeys($handle, $key);
			time_nanosleep(0, 250 * 1000 * 1000);

			// 3. check if subkey is empty
			$r = k2hdkcpx_get_subkeys($handle, $key);
			if(!$r) {
				echo "OK";
			}else{
				echo "NG";
